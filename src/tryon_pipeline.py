        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # GPU-swap strategy: move unet_encoder to GPU only for its forward
                # pass, then immediately return it to CPU.  Both UNets are never
                # resident in VRAM simultaneously, so peak VRAM stays ~4-5 GB
                # instead of ~8.5 GB.  PCIe transfer (~0.2 s) is far faster than
                # running a full SDXL UNet on CPU (~30-60 s per step).
                _main_dev = next(self.unet.parameters()).device
                _enc_home = next(self.unet_encoder.parameters()).device
                if _enc_home != _main_dev:
                    self.unet_encoder.to(_main_dev)
                down, reference_features = self.unet_encoder(
                    cloth, t, text_embeds_cloth, return_dict=False)
                reference_features = list(reference_features)
                if _enc_home != _main_dev:
                    self.unet_encoder.to(_enc_home)
                    torch.cuda.empty_cache()   # release cached VRAM immediately
